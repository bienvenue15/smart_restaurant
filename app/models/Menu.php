...
class Menu extends Model {
    ...
    public function getAllCategoriesWithItems($restaurantId = null) {
        try {
            $query = "SELECT 
                      ...
                      LEFT JOIN menu_items m ON c.uuid = m.category_uuid";
            
            $conditions = ["c.is_active = 1"];
            $params = [];
            
            // Filter by restaurant_uuid for multi-tenancy
            if ($restaurantId) {
                $conditions[] = "c.restaurant_uuid = ?";
                $conditions[] = "(m.restaurant_uuid = ? OR m.uuid IS NULL)";
                $params[] = $restaurantId;
                $params[] = $restaurantId;
            }
     ...
     */
    public function getItemByUuid($itemUuid) {
        try {
            $query = "SELECT * FROM menu_items WHERE uuid = ?";
            $stmt = $this->db->prepare($query);
    ...
    /**
     * Get menu item by ID (legacy - for internal use only)
     */
    ...
    public function updateItemAvailability($itemId, $isAvailable) {
        try {
            $query = "UPDATE menu_items SET is_available = ? WHERE uuid = ?";
            $stmt = $this->db->prepare($query);
    ...
    public function getSpecialItems() {
        try {
            $query = "SELECT m.*, c.name as category_name 
                      FROM menu_items m
                      INNER JOIN menu_categories c ON m.category_uuid = c.uuid
