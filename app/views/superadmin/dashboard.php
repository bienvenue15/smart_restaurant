...
<li><a href="?req=superadmin&action=plans"><i class="fas fa-box-open"></i> Subscription Plans</a></li>
</div>
<div class="menu-section">
<div class="menu-section-title">Communication</div>
<li><a href="?req=superadmin&action=announcements"><i class="fas fa-bullhorn"></i> Announcements</a></li>
</div>
<div class="menu-section">
...
<li><a href="#" data-section="tickets"><i class="fas fa-ticket-alt"></i> Support Tickets <span class="badge" id="ticketBadge">0</span></a></li>
<li><a href="#" data-section="messages"><i class="fas fa-envelope"></i> Messages <span class="badge" id="messageBadge" style="display: none;">0</span></a></li>
...
try {
const response = await fetch(buildApiUrl('get_support_overview', { format: 'json' }));
const result = await response.json();
if (result.status === 'OK' && result.data && result.data.overview) {
const openTickets = result.data.overview.open_tickets || 0;
...
try {
const response = await fetch(buildApiUrl('get_support_messages', { format: 'json' }));
const result = await response.json();
if (result.status === 'OK' && result.data) {
const newMessages = result.data.filter(m => m.status === 'new').length;
const badge = document.getElementById('messageBadge');
if (badge) {
badge.textContent = newMessages;
badge.style.display = newMessages > 0 ? 'inline-block' : 'none';
}
}
} catch (error) {
}
...
const userDropdown1 = document.getElementById('userDropdown');
if (userDropdown1) userDropdown1.classList.remove('active');
}
}
...
const notificationDropdown1 = document.getElementById('notificationDropdown');
if (notificationDropdown1) notificationDropdown1.classList.remove('active');
}
}
function closeDropdowns() {
if (notificationDropdownOpen) {
notificationDropdownOpen = false;
const notificationDropdown2 = document.getElementById('notificationDropdown');
if (notificationDropdown2) notificationDropdown2.classList.remove('active');
}
if (userDropdownOpen) {
userDropdownOpen = false;
const userDropdown2 = document.getElementById('userDropdown');
if (userDropdown2) userDropdown2.classList.remove('active');
}
}
...
<div class="notification-item ${item.is_read == 0 ? 'unread' : ''}" onclick="event.preventDefault(); event.stopPropagation(); handleNotificationClick(${item.id}, event);" style="cursor: pointer;">
...
`).join('') + `
<button type="button" class="btn btn-secondary" style="width: 100%; margin-top: 0.5rem;" onclick="event.preventDefault(); event.stopPropagation(); markAllNotificationsRead(event);">Mark all as read</button>
`;
}
...
const result = await markNotificationAsRead('all');
loadNotificationPreview();
}
async function markNotificationAsRead(id) {
try {
const basePath = getBasePath();
const url = basePath + '?req=superadmin&action=mark_notification_read&notification_id=' + encodeURIComponent(id);
...
body: 'notification_id=' + encodeURIComponent(String(id)),
credentials: 'same-origin'
});
if (!response.ok) {
const errorText = await response.text();
return { status: 'FAIL', message: errorText };
}
const result = await response.json();
if (result.status !== 'OK') {
}
...
try {
const basePath = getBasePath();
const response = await fetch(basePath + '?req=superadmin&action=mark_message_read', {
method: 'POST',
...
const msgIndex = supportMessagesCache.findIndex(m => m.id == messageId);
if (msgIndex !== -1) {
supportMessagesCache[msgIndex].status = 'read';
}
// Re-render the messages table
...
const response = await fetch(buildApiUrl('list_restaurants', { format: 'json' }));
const result = await response.json();
if (result.status === 'OK') {
...
try {
const response = await fetch(buildApiUrl('get_recent_activity', { limit: 10 }));
const result = await response.json();
if (result.status === 'OK') {
...
activityList.innerHTML = '<li style="text-align: center; padding: 2rem; color: #95a5a6;">No recent activity</li>';
return;
}
...
// Ensure BASE_PATH ends with slash for proper URL construction
...
<td>${r.email}</td>
<td><span class="badge-plan" data-plan="${r.subscription_plan}">${r.subscription_plan}</span></td>
...
`).join('');
// Apply dynamic badge colors
applyDynamicBadgeColors();
}
// Fetch and apply badge colors from database
async function applyDynamicBadgeColors() {
try {
const response = await fetch(buildApiUrl('get_active_plans'));
const result = await response.json();
if (result.status === 'OK') {
const planColors = {};
result.data.forEach(plan => {
planColors[plan.plan_name] = plan.badge_color || '#6c757d';
});
// Apply colors to all badge-plan elements
document.querySelectorAll('.badge-plan').forEach(badge => {
const planName = badge.getAttribute('data-plan');
const color = planColors[planName] || '#6c757d';
...
try {
const response = await fetch(buildApiUrl('get_restaurant', { id: id, format: 'json' }));
const result = await response.json();
if (result.status === 'OK') {
const r = result.data;
const stats = r.stats || {};
...
<p class="page-subtitle">Restaurant Details & Statistics</p>
</div>
<div style="display: grid; grid-template-columns: 2fr 1fr; gap: 1.5rem; margin-bottom: 2rem;">
<!-- Main Details -->
<div class="form-card">
<h3 style="margin-bottom: 1.5rem; color: #2c3e50; border-bottom: 2px solid #e0e0e0; padding-bottom: 0.5rem;">
<i class="fas fa-info-circle"></i> Restaurant Information
</h3>
<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
<div>
<label style="font-weight: 600; color: #7f8c8d; font-size: 0.85rem; display: block; margin-bottom: 0.5rem;">Restaurant Name</label>
<div style="font-size: 1rem; color: #2c3e50;">${escapeHtml(r.name)}</div>
</div>
<div>
<label style="font-weight: 600; color: #7f8c8d; font-size: 0.85rem; display: block; margin-bottom: 0.5rem;">Slug</label>
<div style="font-size: 1rem; color: #2c3e50;">${escapeHtml(r.slug)}</div>
</div>
<div>
<label style="font-weight: 600; color: #7f8c8d; font-size: 0.85rem; display: block; margin-bottom: 0.5rem;">Email</label>
<div style="font-size: 1rem; color: #2c3e50;">${escapeHtml(r.email)}</div>
</div>
<div>
<label style="font-weight: 600; color: #7f8c8d; font-size: 0.85rem; display: block; margin-bottom: 0.5rem;">Phone</label>
<div style="font-size: 1rem; color: #2c3e50;">${escapeHtml(r.phone || 'N/A')}</div>
</div>
<div style="grid-column: 1 / -1;">
<label style="font-weight: 600; color: #7f8c8d; font-size: 0.85rem; display: block; margin-bottom: 0.5rem;">Address</label>
<div style="font-size: 1rem; color: #2c3e50;">${escapeHtml(r.address || 'N/A')}</div>
</div>
<div>
<label style="font-weight: 600; color: #7f8c8d; font-size: 0.85rem; display: block; margin-bottom: 0.5rem;">Subscription Plan</label>
<div><span class="badge-plan badge-${r.subscription_plan}">${r.subscription_plan.toUpperCase()}</span></div>
</div>
<div>
<label style="font-weight: 600; color: #7f8c8d; font-size: 0.85rem; display: block; margin-bottom: 0.5rem;">Status</label>
<div><span class="badge-status badge-${r.is_active ? 'active' : 'inactive'}">${r.is_active ? 'Active' : 'Inactive'}</span></div>
</div>
<div>
<label style="font-weight: 600; color: #7f8c8d; font-size: 0.85rem; display: block; margin-bottom: 0.5rem;">Subscription End</label>
<div style="font-size: 1rem; color: #2c3e50;">${r.subscription_end || 'N/A'}</div>
</div>
<div>
<label style="font-weight: 600; color: #7f8c8d; font-size: 0.85rem; display: block; margin-bottom: 0.5rem;">Created</label>
<div style="font-size: 1rem; color: #2c3e50;">${formatDateTime(r.created_at)}</div>
</div>
</div>
<div style="margin-top: 2rem; padding-top: 1.5rem; border-top: 2px solid #e0e0e0; display: flex; gap: 1rem;">
<button class="btn btn-primary" onclick="editRestaurant(${r.id})">
<i class="fas fa-edit"></i> Edit Restaurant
</button>
<button class="btn btn-secondary" onclick="loadSection('dashboard')">
...
<h3 style="margin-bottom: 1rem; color: #2c3e50; border-bottom: 2px solid #e0e0e0; padding-bottom: 0.5rem;">
<i class="fas fa-chart-bar"></i> Statistics
</h3>
<div style="display: grid; gap: 1rem;">
<div style="display: flex; justify-content: space-between; align-items: center;">
<span style="color: #7f8c8d;">Total Users</span>
<strong style="font-size: 1.5rem; color: #3498db;">${stats.total_users || 0}</strong>
</div>
<div style="display: flex; justify-content: space-between; align-items: center;">
<span style="color: #7f8c8d;">Total Tables</span>
<strong style="font-size: 1.5rem; color: #2ecc71;">${stats.total_tables || 0}</strong>
</div>
<div style="display: flex; justify-content: space-between; align-items: center;">
<span style="color: #7f8c8d;">Total Orders</span>
<strong style="font-size: 1.5rem; color: #f39c12;">${stats.total_orders || 0}</strong>
</div>
<div style="display: flex; justify-content: space-between; align-items: center;">
<span style="color: #7f8c8d;">Today Revenue</span>
<strong style="font-size: 1.5rem; color: #9b59b6;">${formatCurrency(stats.today_revenue || 0)}</strong>
</div>
</div>
</div>
<div class="form-card">
...
<div style="display: grid; gap: 1rem;">
<div style="display: flex; justify-content: space-between; align-items: center;">
<span style="color: #7f8c8d;">Max Tables</span>
<strong style="font-size: 1.25rem; color: #2c3e50;">${r.max_tables || 0}</strong>
</div>
<div style="display: flex; justify-content: space-between; align-items: center;">
<span style="color: #7f8c8d;">Max Users</span>
<strong style="font-size: 1.25rem; color: #2c3e50;">${r.max_users || 0}</strong>
</div>
</div>
</div>
</div>
...
showError(result.message || 'Failed to load restaurant details');
}
} catch (error) {
...
if (!confirm(`Are you sure you want to DEACTIVATE "${name}"?\n\nThis will make the restaurant inaccessible but keep all data.`)) {
return;
}
try {
const response = await fetch(buildApiUrl('delete_restaurant'), {
method: 'POST',
headers: { 'Content-Type': 'application/json' },
body: JSON.stringify({ id: id, hard_delete: false })
});
const result = await response.json();
if (result.status === 'OK') {
showSuccess(result.message || 'Restaurant deactivated successfully');
setTimeout(() => loadSection('dashboard'), 1500);
} else {
showError(result.message || 'Failed to delete restaurant');
}
} catch (error) {
...
async function loadAddRestaurantForm() {
// Fetch available plans from database
const plansResponse = await fetch(BASE_PATH + '/?req=superadmin&action=get_active_plans');
const plansData = await plansResponse.json();
const plans = plansData.plans || [];
const planOptionsHTML = plans.map((plan, index) => {
const priceDisplay = plan.price_monthly > 0 
? new Intl.NumberFormat().format(plan.price_monthly) + ' RWF/mo'
: 'Free / ' + plan.duration_days + ' days';
return `
<div class="plan-option">
<input type="radio" name="subscription_plan" id="plan_${plan.plan_name}" value="${plan.plan_name}" ${index === 0 ? 'checked' : ''} required>
<label for="plan_${plan.plan_name}" class="plan-label">
...
<p class="page-subtitle">Create a new restaurant tenant and provide them with credentials</p>
<div style="background: #e8f4fd; border-left: 4px solid #3498db; padding: 15px; margin-bottom: 25px; border-radius: 5px;">
<h4 style="color: #2980b9; margin-bottom: 8px;">
<i class="fas fa-info-circle"></i> Restaurant Registration Process
</h4>
<p style="color: #34495e; margin: 0; line-height: 1.6;">
Restaurants must contact us at <strong>support@smartresto.rw</strong> or call <strong>+250 788 555 000</strong> to request account creation.
As Super Admin, you can create their account here and provide them with login credentials.
</p>
</div>
</div>
...
<input type="text" name="slug" required id="slugInput" title="Only lowercase letters, numbers, and hyphens allowed">
...
<label>TIN Number <span class="required">*</span></label>
<input type="text" name="tin" pattern="[0-9]{9,10}" maxlength="10" placeholder="e.g., 123456789" required>
<div class="help-text">Tax Identification Number (9-10 digits)</div>
</div>
<div class="form-group full-width">
<label>Address</label>
...
const response = await fetch(buildApiUrl('create_restaurant'), {
method: 'POST',
headers: { 'Content-Type': 'application/json' },
body: JSON.stringify(data)
});
const contentType = response.headers.get('content-type');
// Try to parse response as JSON first (even on error)
let result;
try {
...
const debugInfo = result.debug ? `<br><small>Debug: ${result.debug}</small>` : '';
messageContainer.innerHTML = `<div class="error-message"><i class="fas fa-exclamation-circle"></i> ${result.message || 'Failed to create restaurant'}${debugInfo}</div>`;
submitBtn.disabled = false;
submitBtn.innerHTML = '<i class="fas fa-save"></i> Create Restaurant';
}
} catch (error) {
messageContainer.innerHTML = `<div class="error-message"><i class="fas fa-exclamation-circle"></i> ${error.message}</div>`;
submitBtn.disabled = false;
submitBtn.innerHTML = '<i class="fas fa-save"></i> Create Restaurant';
}
});
...
try {
// Fetch restaurant data and plans in parallel
const [restaurantResponse, plansResponse] = await Promise.all([
fetch(buildApiUrl('get_restaurant', { id: id })),
fetch(BASE_PATH + '/?req=superadmin&action=get_active_plans')
]);
...
if (result.status === 'OK') {
const r = result.data;
// Generate plan options dynamically
const editPlanOptionsHTML = plans.map(plan => {
const priceDisplay = plan.price_monthly > 0 
? new Intl.NumberFormat().format(plan.price_monthly) + ' RWF/mo'
: 'Free / ' + plan.duration_days + ' days';
return `
<div class="plan-option">
<input type="radio" name="subscription_plan" id="edit_plan_${plan.plan_name}" value="${plan.plan_name}" ${r.subscription_plan === plan.plan_name ? 'checked' : ''}>
<label for="edit_plan_${plan.plan_name}" class="plan-label">
<div class="plan-name">${plan.display_name}</div>
<div class="plan-price">${priceDisplay}</div>
</label>
</div>
...
<div class="form-group full-width">
<label>TIN Number <span class="required">*</span></label>
<input type="text" name="tin" value="${r.tin || ''}" pattern="[0-9]{9,10}" maxlength="10" placeholder="e.g., 123456789" required>
<div class="help-text">Tax Identification Number (9-10 digits)</div>
</div>
<div class="form-group full-width">
<label>Address</label>
...
// Handle checkbox - must explicitly set to 0 if unchecked
...
try {
const url = buildApiUrl('update_restaurant');
const response = await fetch(url, {
method: 'POST',
headers: { 
...
restaurant: (document.getElementById('filterRestaurant') && document.getElementById('filterRestaurant').value) || '',
role: (document.getElementById('filterRole') && document.getElementById('filterRole').value) || '',
...
// Update stat cards - parse revenue string by removing commas first
const totalRevenue = parseFloat(String(data.overview.total_revenue).replace(/,/g, '')) || 0;
document.getElementById('totalRevenue').textContent = formatCurrency(totalRevenue);
...
return 'Revenue: RWF ' + Math.round(context.parsed.y).toLocaleString();
}
}
}
},
...
async function createPlanChart(data) {
...
// Fetch plan colors from database
let planColors = {...defaultColors};
try {
const response = await fetch(buildApiUrl('get_active_plans'));
const result = await response.json();
if (result.status === 'OK') {
result.data.forEach(plan => {
if (plan.badge_color) {
planColors[plan.plan_name] = plan.badge_color;
}
});
}
} catch (error) {
...
data = [{subscription_plan: 'No data', count: 1}];
planColors['No data'] = '#e0e0e0';
}
...
backgroundColor: data.map(d => planColors[d.subscription_plan] || '#6c757d'),
borderWidth: 2,
borderColor: '#fff'
}]
},
...
callbacks: {
label: function(context) {
return 'Revenue: RWF ' + Math.round(context.parsed.x).toLocaleString() + ' (' + data[context.dataIndex].order_count + ' orders)';
}
}
}
},
...
<button class="export-card" onclick="openCustomReportBuilder()">
...
// Parse revenue values - remove commas and convert to number
const totalRevenue = parseFloat(String(r.total_revenue).replace(/,/g, '')) || 0;
const avgOrderValue = parseFloat(String(r.avg_order_value).replace(/,/g, '')) || 0;
return `
<tr>
<td>
<div class="restaurant-info">
...
<p style="color: #7f8c8d; margin-bottom: 2rem;">Build custom reports with your own filters and date ranges</p>
<form id="customReportForm">
<!-- Report Type -->
<div style="margin-bottom: 1.5rem;">
<label style="display: block; font-weight: 600; margin-bottom: 0.5rem;">Report Type</label>
<select id="reportType" style="width: 100%; padding: 0.75rem; border: 2px solid #e0e0e0; border-radius: 8px;" onchange="updateReportFields()">
...
<div style="margin-bottom: 1.5rem;">
<label style="display: block; font-weight: 600; margin-bottom: 0.5rem;">Restaurant(s)</label>
<select id="reportRestaurants" multiple style="width: 100%; padding: 0.75rem; border: 2px solid #e0e0e0; border-radius: 8px; height: 120px;">
...
<input type="date" id="reportStartDate" style="width: 100%; padding: 0.75rem; border: 2px solid #e0e0e0; border-radius: 8px;" value="${new Date(Date.now() - 30*24*60*60*1000).toISOString().split('T')[0]}">
</div>
<div>
<label style="display: block; font-weight: 600; margin-bottom: 0.5rem;">End Date</label>
<input type="date" id="reportEndDate" style="width: 100%; padding: 0.75rem; border: 2px solid #e0e0e0; border-radius: 8px;" value="${new Date().toISOString().split('T')[0]}">
</div>
</div>
<!-- Quick Presets -->
<div style="margin-bottom: 1.5rem;">
<label style="display: block; font-weight: 600; margin-bottom: 0.5rem;">Quick Presets</label>
<div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
<button type="button" class="btn btn-secondary" onclick="setDateRange('today')">Today</button>
<button type="button" class="btn btn-secondary" onclick="setDateRange('yesterday')">Yesterday</button>
...
<button type="button" class="btn btn-secondary" onclick="setDateRange('quarter')">This Quarter</button>
<button type="button" class="btn btn-secondary" onclick="setDateRange('year')">This Year</button>
</div>
</div>
<!-- Additional Filters -->
<div id="additionalFilters" style="margin-bottom: 1.5rem;"></div>
<!-- Group By -->
<div style="margin-bottom: 1.5rem;">
<label style="display: block; font-weight: 600; margin-bottom: 0.5rem;">Group By</label>
<select id="reportGroupBy" style="width: 100%; padding: 0.75rem; border: 2px solid #e0e0e0; border-radius: 8px;">
...
<label style="display: block; font-weight: 600; margin-bottom: 0.5rem;">Export Format</label>
<div style="display: flex; gap: 1rem;">
<label style="display: flex; align-items: center; cursor: pointer;">
<input type="radio" name="exportFormat" value="csv" checked style="margin-right: 0.5rem;">
<i class="fas fa-file-csv" style="margin-right: 0.25rem;"></i> CSV
</label>
<label style="display: flex; align-items: center; cursor: pointer;">
<input type="radio" name="exportFormat" value="excel" style="margin-right: 0.5rem;">
<i class="fas fa-file-excel" style="margin-right: 0.25rem;"></i> Excel
</label>
<label style="display: flex; align-items: center; cursor: pointer;">
<input type="radio" name="exportFormat" value="pdf" style="margin-right: 0.5rem;">
<i class="fas fa-file-pdf" style="margin-right: 0.25rem;"></i> PDF
</label>
</div>
</div>
<!-- Actions -->
<div style="display: flex; gap: 1rem; justify-content: flex-end; padding-top: 1rem; border-top: 2px solid #e0e0e0;">
<button type="button" class="btn btn-secondary" onclick="closeModal()">Cancel</button>
<button type="button" class="btn btn-primary" onclick="previewCustomReport()">
...
`;
showModal(html);
loadRestaurantListForReport();
}
async function loadRestaurantListForReport() {
try {
const response = await fetch(buildApiUrl('list_restaurants', { format: 'json' }));
const result = await response.json();
if (result.status === 'OK') {
const select = document.getElementById('reportRestaurants');
...
endDateInput.value = new Date(today - 24*60*60*1000).toISOString().split('T')[0];
break;
case 'week':
...
case 'revenue':
html = `
<label style="display: block; font-weight: 600; margin-bottom: 0.5rem;">Minimum Revenue</label>
<input type="number" id="minRevenue" placeholder="Optional" style="width: 100%; padding: 0.75rem; border: 2px solid #e0e0e0; border-radius: 8px;">
`;
break;
case 'orders':
html = `
<label style="display: block; font-weight: 600; margin-bottom: 0.5rem;">Order Status</label>
<select id="orderStatus" style="width: 100%; padding: 0.75rem; border: 2px solid #e0e0e0; border-radius: 8px;">
...
case 'staff':
html = `
<label style="display: block; font-weight: 600; margin-bottom: 0.5rem;">Staff Role</label>
<select id="staffRole" style="width: 100%; padding: 0.75rem; border: 2px solid #e0e0e0; border-radius: 8px;">
<option value="">All Roles</option>
<option value="admin">Admin</option>
<option value="manager">Manager</option>
<option value="waiter">Waiter</option>
...
showSuccess('Generating preview...');
// In a real implementation, this would call an API to generate preview data
setTimeout(() => {
const previewHtml = `
<div style="padding: 2rem;">
<h3 style="margin-bottom: 1rem;">Report Preview</h3>
<p style="color: #7f8c8d; margin-bottom: 1.5rem;">Preview of your custom report</p>
<div style="background: #f8f9fa; padding: 1rem; border-radius: 8px; margin-bottom: 1rem;">
<strong>Report Type:</strong> ${reportData.type}<br>
<strong>Date Range:</strong> ${reportData.startDate} to ${reportData.endDate}<br>
<strong>Restaurants:</strong> ${reportData.restaurants.length} selected<br>
<strong>Group By:</strong> ${reportData.groupBy}
</div>
<div style="padding: 2rem; text-align: center; color: #7f8c8d; border: 2px dashed #e0e0e0; border-radius: 8px;">
<i class="fas fa-chart-bar fa-3x" style="margin-bottom: 1rem; opacity: 0.5;"></i>
<p>Preview chart and data table would appear here</p>
</div>
<div style="display: flex; gap: 1rem; justify-content: flex-end; margin-top: 1.5rem;">
<button class="btn btn-secondary" onclick="closeModal()">Close</button>
<button class="btn btn-primary" onclick="closeModal(); openCustomReportBuilder();">Back to Builder</button>
</div>
</div>
`;
...
restaurants: reportData.restaurants.join(','),
group_by: reportData.groupBy,
format: reportData.format
});
// Add additional filters
Object.keys(reportData.filters).forEach(key => {
if (reportData.filters[key]) {
params.append(key, reportData.filters[key]);
}
});
// Trigger download
window.location.href = getBasePath() + '/?' + params.toString();
showSuccess('Report is being generated and will download shortly...');
closeModal();
}
function collectReportData() {
const restaurants = Array.from(document.getElementById('reportRestaurants').selectedOptions).map(o => o.value);
const format = document.querySelector('input[name="exportFormat"]:checked').value;
const type = document.getElementById('reportType').value;
// Collect additional filters
const filters = {};
const filtersDiv = document.getElementById('additionalFilters');
const filterInputs = filtersDiv.querySelectorAll('input, select');
...
auditLogState.search = auditSearchInput ? auditSearchInput.value.trim() : '';
const auditActionFilter = document.getElementById('auditActionFilter');
auditLogState.action = auditActionFilter ? auditActionFilter.value : '';
const auditStartDate = document.getElementById('auditStartDate');
auditLogState.startDate = auditStartDate ? auditStartDate.value : '';
const auditEndDate = document.getElementById('auditEndDate');
auditLogState.endDate = auditEndDate ? auditEndDate.value : '';
auditLogState.page = 1;
loadAuditLogData();
}
...
async function viewTicket(id) { try { const basePath = getBasePath(); const response = await fetch(basePath + `?req=superadmin&action=get_ticket_details&ticket_id=${id}&format=json`); const result = await response.json(); if (result.status === 'OK') { renderTicketDetail(result.data); } else { showError(result.message || 'Ticket not found'); } } catch (error) {  } }
...
async function updateTicketStatus(ticketId) { const ticketStatusSelect = document.getElementById('ticketStatusSelect'); const status = ticketStatusSelect ? ticketStatusSelect.value : ''; if (!status) return; try { const basePath = getBasePath(); const response = await fetch(basePath + '?req=superadmin&action=update_ticket_status', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ticket_id: ticketId, status }) }); const result = await response.json(); if (result.status === 'OK') { showSuccess('Ticket updated'); loadSupportData(); viewTicket(ticketId); } else { showError(result.message || 'Failed to update ticket'); } } catch (error) {  } }
async function submitTicketReply(event, ticketId) { event.preventDefault(); const input = document.getElementById('ticketReplyInput'); if (!input || !input.value.trim()) return; try { const basePath = getBasePath(); const response = await fetch(basePath + '?req=superadmin&action=reply_ticket', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ticket_id: ticketId, message: input.value.trim() }) }); const result = await response.json(); if (result.status === 'OK') { input.value = ''; viewTicket(ticketId); } else { showError(result.message || 'Failed to send reply'); } } catch (error) {  } }
function applyTicketFilters() { const ticketStatusFilter = document.getElementById('ticketStatusFilter'); supportTicketState.status = ticketStatusFilter ? ticketStatusFilter.value : ''; const ticketPriorityFilter = document.getElementById('ticketPriorityFilter'); supportTicketState.priority = ticketPriorityFilter ? ticketPriorityFilter.value : ''; const ticketSearchInput = document.getElementById('ticketSearchInput'); supportTicketState.search = ticketSearchInput ? ticketSearchInput.value.trim() : ''; supportTicketState.page = 1; loadSupportData(); }
...
</div>`; document.getElementById('mainContent').innerHTML = html; try { const basePath = getBasePath(); const response = await fetch(basePath + '?req=superadmin&action=get_support_messages&format=json'); const result = await response.json(); if (result.status === 'OK') { supportMessagesCache = result.data || []; renderSupportMessages(); } else { showError(result.message || 'Failed to load messages'); } } catch (error) {  } }
function renderSupportMessages() { const tbody = document.getElementById('messagesTableBody'); if (!tbody) return; if (!supportMessagesCache.length) { tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:2rem;color:#95a5a6;">No messages available</td></tr>'; return; } tbody.innerHTML = supportMessagesCache.map(msg => `
<tr>
<td>
...
<td><div style="max-width:300px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${escapeHtml(msg.message || '')}">${escapeHtml(msg.message || '')}</div></td>
...
<td>
${msg.status === 'new' ? `<button onclick="markMessageAsRead(${msg.id})" style="background:linear-gradient(135deg, #10b981 0%, #059669 100%);color:white;border:none;padding:8px 16px;border-radius:8px;cursor:pointer;font-size:0.85rem;font-weight:600;box-shadow:0 2px 8px rgba(16,185,129,0.3);transition:all 0.3s ease;" onmouseover="this.style.transform='translateY(-1px)';this.style.boxShadow='0 4px 12px rgba(16,185,129,0.4)'" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='0 2px 8px rgba(16,185,129,0.3)'" title="Mark as read"><i class="fas fa-check-circle"></i> Mark Read</button>` : '<span style="color:#95a5a6;font-size:0.85rem;"><i class="fas fa-check" style="margin-right:4px;"></i>Read</span>'}
</td>
</tr>`).join(''); }
...
</div>`; document.getElementById('mainContent').innerHTML = html; try { const basePath = getBasePath(); const response = await fetch(basePath + '?req=superadmin&action=get_notifications&limit=40&format=json'); const result = await response.json(); if (result.status === 'OK') { notificationFeedCache = result.data || []; updateNotificationBadge(result.unread_count || 0); renderNotificationCenter(); } else { showError(result.message || 'Failed to load notifications'); } } catch (error) {  } }
...
if (!form) return; 
// Support & Contact
form.support_email.value = (settings.support_email && settings.support_email.value) || 'info@inovasiyo.rw';
form.support_phone.value = (settings.support_phone && settings.support_phone.value) || '+250 788 000 999';
form.default_timezone.value = (settings.default_timezone && settings.default_timezone.value) || 'Africa/Kigali';
form.maintenance_mode.value = (settings.maintenance_mode && settings.maintenance_mode.value) || 'off';
// Security & Sessions
form.session_timeout.value = (settings.session_timeout && settings.session_timeout.value) || '7200';
form.max_login_attempts.value = (settings.max_login_attempts && settings.max_login_attempts.value) || '5';
form.password_min_length.value = (settings.password_min_length && settings.password_min_length.value) || '8';
form.staff_clock_in_required.value = (settings.staff_clock_in_required && settings.staff_clock_in_required.value) || 'on';
// Business Hours
const businessHours = (settings.business_hours && settings.business_hours.value) || '{}';
let hours = {};
try {
...
// Order Management
form.max_pending_orders.value = (settings.max_pending_orders && settings.max_pending_orders.value) || '50';
form.auto_assign_orders.value = (settings.auto_assign_orders && settings.auto_assign_orders.value) || 'off';
form.order_timeout.value = (settings.order_timeout && settings.order_timeout.value) || '300';
form.minimum_order_amount.value = (settings.minimum_order_amount && settings.minimum_order_amount.value) || '0';
// Table Management
form.max_tables_per_restaurant.value = (settings.max_tables_per_restaurant && settings.max_tables_per_restaurant.value) || '100';
form.table_auto_release.value = (settings.table_auto_release && settings.table_auto_release.value) || 'on';
form.table_release_timeout.value = (settings.table_release_timeout && settings.table_release_timeout.value) || '3600';
form.staff_shift_max_hours.value = (settings.staff_shift_max_hours && settings.staff_shift_max_hours.value) || '12';
// Notifications
form.notifications_enabled.value = (settings.notifications_enabled && settings.notifications_enabled.value) || 'on';
form.email_notifications.value = (settings.email_notifications && settings.email_notifications.value) || 'on';
// Backup
form.backup_schedule.value = (settings.backup_schedule && settings.backup_schedule.value) || '02:00 Africa/Kigali';
form.backup_retention_days.value = (settings.backup_retention_days && settings.backup_retention_days.value) || '30';
// QR Code
form.qrcode_format.value = (settings.qrcode_format && settings.qrcode_format.value) || 'png';
form.qrcode_size.value = (settings.qrcode_size && settings.qrcode_size.value) || '300';
}
...
<button type="button" class="btn btn-primary" onclick="triggerBackupRequest(event)"><i class="fas fa-database"></i> Trigger Backup</button>
</div>`; document.getElementById('mainContent').innerHTML = html; }
async function triggerBackupRequest(event) {
if (event) {
event.preventDefault();
event.stopPropagation();
}
if (!confirm('Trigger manual backup now? This may take a few moments.')) return;
try {
const basePath = getBasePath();
const response = await fetch(basePath + '?req=superadmin&action=trigger_backup', { 
method: 'POST',
headers: {
'Content-Type': 'application/json',
'X-Requested-With': 'XMLHttpRequest'
...
</div>`; document.getElementById('mainContent').innerHTML = html; try { const basePath = getBasePath(); const response = await fetch(basePath + '?req=superadmin&action=get_system_logs&format=json'); const result = await response.json(); if (result.status === 'OK') { renderSystemLogs(result.data || []); } else { showError(result.message || 'Failed to load logs'); } } catch (error) {  } }
...
const result = await response.json();
if (result.status === 'OK') {
displaySubscriptionsPage(result.data);
} else {
...
async function displaySubscriptionsPage(restaurants) {
// Fetch plans for dynamic stats
let planStats = [];
try {
const response = await fetch(buildApiUrl('get_active_plans'));
const result = await response.json();
if (result.status === 'OK') {
planStats = result.data.map(plan => ({
name: plan.display_name,
plan_name: plan.plan_name,
color: plan.badge_color || '#6c757d',
count: restaurants.filter(r => r.subscription_plan === plan.plan_name).length
}));
}
} catch (error) {
}
const planStatsHtml = planStats.map(plan => `
<div class="stat-card" style="border-left: 4px solid ${plan.color};">
<div class="stat-header">
<div class="stat-title">${plan.name}</div>
<div class="stat-icon" style="color: ${plan.color};"><i class="fas fa-layer-group"></i></div>
</div>
<div class="stat-value">${plan.count}</div>
</div>
`).join('');
const html = `
<div class="page-header">
<h1 class="page-title">Subscription Management</h1>
<p class="page-subtitle">Manage restaurant subscriptions, billing, and renewals</p>
</div>
<div id="messageContainer"></div>
<!-- Subscription Stats -->
...
<div class="stat-value">${restaurants.filter(r => r.is_active).length}</div>
</div>
${planStatsHtml}
...
<div class="stat-value">${restaurants.filter(r => {
...
<h3 class="section-title"><i class="fas fa-list"></i> All Subscriptions</h3>
<input type="text" id="subscriptionSearch" placeholder="Search restaurants..." style="padding: 0.5rem 1rem; border: 2px solid #e0e0e0; border-radius: 8px; width: 300px;">
</div>
...
const statusClass = daysLeft < 0 ? 'expired' : daysLeft <= 7 ? 'expiring' : 'active';
const statusText = daysLeft < 0 ? 'Expired' : daysLeft <= 7 ? 'Expiring Soon' : 'Active';
return `
<tr data-restaurant="${r.name.toLowerCase()}">
<td>
<div style="display: flex; align-items: center; gap: 0.75rem;">
<div style="width: 40px; height: 40px; border-radius: 8px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); display: flex; align-items: center; justify-content: center; color: white; font-weight: 600;">
${r.name.substring(0, 2).toUpperCase()}
</div>
<div>
<div style="font-weight: 600;">${escapeHtml(r.name)}</div>
<div style="font-size: 0.85rem; color: #7f8c8d;">${escapeHtml(r.email)}</div>
</div>
</div>
</td>
<td><span class="badge-plan" data-plan="${r.subscription_plan}">${r.subscription_plan.toUpperCase()}</span></td>
<td>${r.subscription_start || 'N/A'}</td>
<td>${r.subscription_end || 'N/A'}</td>
<td><span class="subscription-status ${statusClass}">${statusText}</span></td>
<td style="font-weight: 600; color: ${daysLeft < 0 ? '#e74c3c' : daysLeft <= 7 ? '#f39c12' : '#27ae60'};">
${daysLeft < 0 ? 'Expired' : daysLeft + ' days'}
</td>
<td>
<div class="action-buttons">
<button class="btn-icon btn-view" title="Upgrade Plan" onclick="upgradePlan(${r.id}, '${escapeHtml(r.name)}')">
<i class="fas fa-arrow-up"></i>
</button>
<button class="btn-icon btn-edit" title="Extend Subscription" onclick="extendSubscription(${r.id}, '${escapeHtml(r.name)}')">
<i class="fas fa-calendar-plus"></i>
</button>
<button class="btn-icon btn-view" title="View Details" onclick="viewSubscriptionDetails(${r.id})">
<i class="fas fa-eye"></i>
</button>
</div>
</td>
...
rows.forEach(row => {
const restaurantName = row.dataset.restaurant;
row.style.display = restaurantName.includes(searchTerm) ? '' : 'none';
});
});
}
async function upgradePlan(id, name) {
...
const planOptionsHTML = plans.map((plan, index) => {
const priceDisplay = plan.price_monthly > 0 
? new Intl.NumberFormat().format(plan.price_monthly) + ' RWF/month'
: 'Free / ' + plan.duration_days + ' days';
return `
<label style="display: flex; align-items: center; padding: 1rem; border: 2px solid #e0e0e0; border-radius: 8px; cursor: pointer;">
<input type="radio" name="upgradePlan" value="${plan.plan_name}" style="margin-right: 1rem;" ${index === 0 ? 'checked' : ''}>
<div style="flex: 1;">
<div style="font-weight: 600;">${plan.display_name}</div>
<div style="font-size: 0.85rem; color: #7f8c8d;">${priceDisplay}</div>
</div>
</label>
`;
}).join('');
...
<h3 style="margin-bottom: 1rem;">Upgrade Subscription Plan</h3>
<p style="margin-bottom: 1.5rem; color: #7f8c8d;">Upgrade subscription plan for <strong>${name}</strong></p>
<div style="display: grid; gap: 1rem; margin-bottom: 1.5rem;">
${planOptionsHTML}
</div>
<div style="display: flex; gap: 1rem; justify-content: flex-end;">
<button class="btn btn-secondary" onclick="closeModal()">Cancel</button>
<button class="btn btn-primary" onclick="confirmUpgrade(${id})">Upgrade Plan</button>
</div>
</div>
`;
showModal(html);
...
const selectedPlan = document.querySelector('input[name="upgradePlan"]:checked');
if (!selectedPlan) {
showError('Please select a plan');
return;
}
try {
const response = await fetch(buildApiUrl('update_restaurant'), {
method: 'POST',
headers: { 'Content-Type': 'application/json' },
body: JSON.stringify({ 
id: id, 
...
function extendSubscription(id, name) {
const html = `
<div style="padding: 1.5rem;">
<h3 style="margin-bottom: 1rem;">Extend Subscription</h3>
<p style="margin-bottom: 1.5rem; color: #7f8c8d;">Extend subscription for <strong>${name}</strong></p>
<div style="margin-bottom: 1.5rem;">
<label style="display: block; font-weight: 600; margin-bottom: 0.5rem;">Extend by:</label>
<select id="extendMonths" style="width: 100%; padding: 0.75rem; border: 2px solid #e0e0e0; border-radius: 8px;">
...
<div style="display: flex; gap: 1rem; justify-content: flex-end;">
<button class="btn btn-secondary" onclick="closeModal()">Cancel</button>
<button class="btn btn-primary" onclick="confirmExtend(${id})">Extend Subscription</button>
</div>
</div>
`;
showModal(html);
}
async function confirmExtend(id) {
const months = parseInt(document.getElementById('extendMonths').value);
try {
const response = await fetch(buildApiUrl('extend_subscription'), {
method: 'POST',
headers: { 'Content-Type': 'application/json' },
...
modal.innerHTML = `<div style="background: white; border-radius: 12px; max-width: 500px; width: 90%;">${html}</div>`;
