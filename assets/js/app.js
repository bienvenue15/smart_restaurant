...
let cartExpanded = true; // Start expanded on desktop
// Translation helper function
function __(key) {
if (typeof dynamicLangSwitcher !== 'undefined' && dynamicLangSwitcher.getTranslation) {
return dynamicLangSwitcher.getTranslation(key);
}
...
// ===================================
/**
 * Toggle order panel (minimize/expand)
 */
function toggleOrderPanel() {
    const orderWidget = document.getElementById('orderWidget');
    const toggleIcon = document.getElementById('cartToggleIcon');
    const minimizeBtn = orderWidget ? orderWidget.querySelector('.minimize-btn') : null;
    
    if (!orderWidget || !toggleIcon) return;
    
    const isCollapsed = orderWidget.classList.contains('collapsed');
    
    if (isCollapsed) {
        // Expand
        orderWidget.classList.remove('collapsed');
        toggleIcon.classList.remove('fa-chevron-up');
        toggleIcon.classList.add('fa-chevron-down');
        cartExpanded = true;
        // Force button visibility when expanded
        if (minimizeBtn) {
            minimizeBtn.style.cssText = 'display: flex !important; visibility: visible !important; opacity: 1 !important;';
        }
    } else {
        // Collapse
        orderWidget.classList.add('collapsed');
        toggleIcon.classList.remove('fa-chevron-down');
        toggleIcon.classList.add('fa-chevron-up');
        cartExpanded = false;
        // Hide button when collapsed
        if (minimizeBtn) {
            minimizeBtn.style.cssText = 'display: none !important;';
        }
    }
}

/**
 * Initialize order panel state on page load
 */
function initializeOrderPanel() {
    ...
    if (window.innerWidth <= 768) {
        ...
        toggleIcon.classList.add('fa-chevron-up');
        cartExpanded = false;
        // Hide minimize button when collapsed
        if (minimizeBtn) {
            minimizeBtn.style.cssText = 'display: none !important;';
        }
    } else {
        ...
        toggleIcon.classList.add('fa-chevron-down');
        cartExpanded = true;
        // Force minimize button visibility on desktop
        if (minimizeBtn) {
            minimizeBtn.style.cssText = 'display: flex !important; visibility: visible !important; opacity: 1 !important;';
        }
    }
...
// Initialize on load
document.addEventListener('DOMContentLoaded', initializeOrderPanel);
...
*/
function customerCancelOrder(orderUuid, orderNumber) {
if (!confirm(`Are you sure you want to cancel order ${orderNumber}?\n\nThis action cannot be undone.`)) {
return;
}
fetch(`${baseUrl}/api/customer_cancel_order`, {
method: 'POST',
headers: {
...
if (typeof dynamicLangSwitcher !== 'undefined' && dynamicLangSwitcher.getTranslation) {
const translation = dynamicLangSwitcher.getTranslation('messages.order_cancelled');
if (translation && !translation.includes('messages.')) msg = translation;
}
...
const failMsg = (typeof dynamicLangSwitcher !== 'undefined' && dynamicLangSwitcher.getTranslation) 
? dynamicLangSwitcher.getTranslation('messages.cancel_failed') 
: 'Failed to cancel order';
const errorMsg = data.message || failMsg;
showNotification('✗ ' + errorMsg, 'error');
}
})
.catch(error => {
const msg = (typeof dynamicLangSwitcher !== 'undefined' && dynamicLangSwitcher.getTranslation) 
? dynamicLangSwitcher.getTranslation('messages.cancel_failed') 
: 'Failed to cancel order';
...
if (typeof dynamicLangSwitcher !== 'undefined' && dynamicLangSwitcher.getTranslation) {
const translation = dynamicLangSwitcher.getTranslation('messages.item_not_found');
if (translation && !translation.includes('messages.')) msg = translation;
}
showNotification(msg, 'error');
return;
...
updateCartUI();
    let addedText = 'added to your order';
if (typeof dynamicLangSwitcher !== 'undefined' && dynamicLangSwitcher.getTranslation) {
const translation = dynamicLangSwitcher.getTranslation('messages.added_to_order');
// Only use translation if it's not the key itself
if (translation && !translation.includes('messages.')) {
addedText = translation;
}
}
showNotification(`✓ ${itemData.name} ${addedText}`, 'success');
}
/**
...
// Auto-expand panel when items are added (mobile only)
if (cart.length > 0 && window.innerWidth <= 768 && !cartExpanded) {
const orderWidget = document.getElementById('orderWidget');
if (orderWidget && orderWidget.classList.contains('collapsed')) {
toggleOrderPanel();
}
}
...
<p data-translate="orders.order_empty">Your order is empty</p>
<p class="hint" data-translate="orders.order_empty_hint">Browse our menu and add items you'd like to order</p>
</div>
`;
...
if (!tableData) {
let msg = 'Table information not found';
if (typeof dynamicLangSwitcher !== 'undefined' && dynamicLangSwitcher.getTranslation) {
const translation = dynamicLangSwitcher.getTranslation('messages.table_not_found');
if (translation && !translation.includes('messages.')) msg = translation;
}
showNotification(msg, 'error');
return;
...
if (typeof dynamicLangSwitcher !== 'undefined' && dynamicLangSwitcher.getTranslation) {
const translation = dynamicLangSwitcher.getTranslation('messages.cart_empty');
if (translation && !translation.includes('messages.')) msg = translation;
}
showNotification(msg, 'error');
return;
...
showLoading(true);
try {
const response = await fetch(`${baseUrl}/api/create_order`, {
method: 'POST',
headers: {
'Content-Type': 'application/json'
},
...
let failMsg = 'Failed to place order';
if (typeof dynamicLangSwitcher !== 'undefined' && dynamicLangSwitcher.getTranslation) {
const translation = dynamicLangSwitcher.getTranslation('messages.order_failed');
if (translation && !translation.includes('messages.')) failMsg = translation;
}
showNotification(result.message || failMsg, 'error');
}
} catch (error) {
showLoading(false);
let msg = 'Network error. Please try again.';
if (typeof dynamicLangSwitcher !== 'undefined' && dynamicLangSwitcher.getTranslation) {
const translation = dynamicLangSwitcher.getTranslation('messages.network_error');
if (translation && !translation.includes('messages.')) msg = translation;
}
showNotification(msg, 'error');
}
...
showLoading(true);
try {
const response = await fetch(`${baseUrl}/api/call_waiter`, {
method: 'POST',
headers: {
'Content-Type': 'application/json'
},
...
const itemDescription = itemDescElement ? itemDescElement.textContent.toLowerCase() : '';
const dietaryElement = item.querySelector('.dietary-tags');
const dietaryInfo = dietaryElement ? dietaryElement.textContent.toLowerCase() : '';
...
const appAnimationStyle = document.createElement('style');
appAnimationStyle.textContent = `
...
`;
document.head.appendChild(appAnimationStyle);
// ===================================
// EVENT LISTENERS
// ===================================
...
try {
const response = await fetch(`${baseUrl}/api/get_order_history?limit=10`);
const result = await response.json();
...
<button class="btn-cancel-order" onclick="cancelOrder('${order.uuid}')" 
data-order-uuid="${order.uuid}"
data-remaining="${order.cancel_seconds_remaining}">
<i class="fas fa-times-circle"></i> Cancel Order 
(<span class="countdown-${order.uuid}">${order.cancel_seconds_remaining}</span>s)
</button>
</div>
` : ''}
</div>
...
startCancellationCountdown(order.uuid, order.cancel_seconds_remaining);
}
});
}
/**
...
showLoading(true);
try {
const response = await fetch(`${baseUrl}/api/cancel_order`, {
method: 'POST',
headers: {
'Content-Type': 'application/json'
},
...
/**
* Helper function for add items confirmation
*/
function showAddItemsConfirmButton() {
// Update the panel count if it exists
if (typeof customerMenu !== 'undefined' && customerMenu) {
customerMenu.updateAddItemsPanelCount();
}
}
/**
